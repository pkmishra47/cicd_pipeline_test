...
@Tags({"daxoperation", "dataman", "cleanup", "bills"})
@CapabilityDescription("")
@SeeAlso()
@ReadsAttributes({@ReadsAttribute(attribute = "")})
@WritesAttributes({@WritesAttribute(attribute = "")})

public class CleanupBills extends AbstractProcessor {
    private static final String processorName = "CleanupBills";
    ...
    public String getProcessorName() {
        return CleanupBills.processorName;
    }
    ...
    public Gson getGson() {
        if (CleanupBills.gson == null) {
            CleanupBills.gson = GsonUtil.getGson();
        }
        return CleanupBills.gson;
    }
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        try {
            ...
            dbFiles = appDb.getCollection(SiteCollections.bill_group.name());
            cur = dbFiles.find();

            getLogUtil().logMessage(logMetaData, LogStatus.SUCCESS, LogType.GENERAL, String.format("CleanupBills: %s Before cleaning Bills: %s", siteDetails.getSiteName(), dbFiles.count()), Level.INFO, null);
            ...
            while (cur.hasNext()) {
                ...
                if(parts.length == 0) {
                    getLogUtil().logMessage(logMetaData, LogStatus.SUCCESS, LogType.ACTIVITY_STATUS,String.format("CleanupBills: Unable to get the HealthCheck date parsed therefore removing : %s", fileName), Level.INFO, null);
                    dbFiles.remove(dbOrig);
                    continue;
                }

                if(parts.length < 6) {
                    getLogUtil().logMessage(logMetaData, LogStatus.SUCCESS, LogType.ACTIVITY_STATUS,String.format("CleanupBills: Wrong file name %s", fileName), Level.INFO, null);
                    dbFiles.remove(dbOrig);
                    continue;
                }
                ...
                try{
                    ...
                    getLogUtil().logMessage(logMetaData, LogStatus.SUCCESS, LogType.ACTIVITY_STATUS,String.format("CleanupBills: Unable to get the Bill date parsed : %s", fileName), Level.INFO, null);
                    dbFiles.remove(dbOrig);
                    continue;
                }
                
                if(dbillDate != null && daysBetween(dbillDate, today) < 25) {
                    continue;
                }
            ...
            markProcessorFailure(session, inputFF, logMetaData, null, String.format("Exception while cleaning bills : %s", Utility.stringifyException(ex)));
            return;
        } finally {
            if (cur != null) {
                cur.close();
